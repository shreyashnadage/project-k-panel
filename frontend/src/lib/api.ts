...
export function scopedApi(apiKey: string) {
  ...
  return {
    ...
    // ── Test mode ──────────────────────────────────────────────
    getTestMode: async (): Promise<TestModeState> => {
      const { data } = await client.get<TestModeState>('/v1/test/mode')
      return data
    },
    setTestMode: async (mode: 'live' | 'test', simulationId?: number): Promise<TestModeState> => {
      const { data } = await client.post<TestModeState>('/v1/test/mode', {
        mode,
    ...
    getSimulations: async (): Promise<SimulationInfo[]> => {
      const { data } = await client.get<SimulationInfo[]>('/v1/test/simulations')
      return data
    },
    ingestSimData: async (simulationId: number): Promise<IngestResult> => {
      const { data } = await client.post<IngestResult>('/v1/test/ingest', { simulation_id: simulationId })
      return data
    },
    getTestDatasets: async (): Promise<DatasetSummary[]> => {
      const { data } = await client.get<DatasetSummary[]>('/v1/test/datasets')
      return data
    },
    purgeTestDataset: async (simulationId: number): Promise<{ deleted: Record<string, number> }> => {
      const { data } = await client.delete(`/v1/test/datasets/${simulationId}`)
      return data
    },
  }
}
