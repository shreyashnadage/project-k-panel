  ...
  if (!clientApi) {
    return (
      <div className="page-enter" style={{ textAlign: 'center', padding: 64 }}>
        ...
        <p style={{ color: '#a8b8c8', fontSize: 14, marginTop: 8 }}>This client needs a registered device before ledger data is available.</p>
      </div>
    )
  }

  if (error) return <div style={{ textAlign: 'center', padding: 64 }}><p style={{ color: '#c45c4a' }}>Failed to load ledgers.</p></div>

  return (
    <div className="page-enter" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h2 style={{ fontWeight: 700, fontSize: 28, color: '#f5f0e8', margin: 0 }}>Ledgers</h2>
          <p style={{ color: '#a8b8c8', fontSize: 14, marginTop: 4 }}>Chart of accounts for {companyName}</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          {data?.data && data.data.length > 0 && (
            <button onClick={() => downloadCSV(data.data, `ledgers-${companyName}.csv`)}
              style={{ padding: '6px 12px', borderRadius: 10, border: '1px solid #2d3e50', background: '#1b263b', color: '#a8b8c8', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: 13 }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        <form onSubmit={handleSearch} style={{ display: 'flex', flex: 1, minWidth: 220, position: 'relative' }}>
          ...
          <input value={searchInput} onChange={e => setSearchInput(e.target.value)} placeholder="Search by name or group..."
            style={{ flex: 1, padding: '10px 12px 10px 36px', borderRadius: 10, border: '1px solid #2d3e50', background: '#0d1b2a', color: '#f5f0e8', fontSize: 14, outline: 'none' }} />
        </form>
        <select value={parentFilter} onChange={e => { setParentFilter(e.target.value); setSkip(0) }}
          style={{ padding: '10px 14px', borderRadius: 10, border: '1px solid #2d3e50', background: '#0d1b2a', color: '#f5f0e8', fontSize: 14, minWidth: 180, cursor: 'pointer', outline: 'none' }}>
      ...
      <div style={{ background: '#1b263b', border: '1px solid #2d3e50', borderRadius: 14, overflow: 'hidden' }}>
        {isLoading ? (
          <div style={{ padding: 48 }}>{[1, 2, 3, 4, 5].map(i => <div key={i} className="skeleton" style={{ height: 44, background: '#2d3e50', marginBottom: 2 }} />)}</div>
        ) : !data?.data.length ? (
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid #2d3e50' }}>
                  {['Ledger Name', 'Group', 'Type', 'Opening Balance', 'Closing Balance'].map(h => (
                    <th key={h} style={{ padding: '12px 16px', textAlign: 'left', color: '#a8b8c8', fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {data.data.map(l => (
                  <tr key={l.id} style={{ borderBottom: '1px solid #1b263b' }}
                    ...
                    <td style={{ padding: '12px 16px' }}>{l.ledger_type ? <span style={{ padding: '3px 10px', borderRadius: 6, fontSize: 12, fontWeight: 600, background: '#152a40', color: '#3db8a9', border: '1px solid #2d3e5033' }}>{l.ledger_type}</span> : '-'}</td>
                    <td style={{ padding: '12px 16px', fontFamily: 'var(--font-mono)', fontSize: 13, color: '#a8b8c8', textAlign: 'right' }}>{fmtBalance(l.opening_balance)}</td>
                    <td style={{ padding: '12px 16px', fontFamily: 'var(--font-mono)', fontSize: 13, color: '#f5f0e8', textAlign: 'right', fontWeight: 500 }}>{fmtBalance(l.closing_balance)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {totalPages > 1 && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderTop: '1px solid #2d3e50' }}>
            <span style={{ color: '#a8b8c8', fontSize: 13 }}>{skip + 1}–{Math.min(skip + PAGE_SIZE, total)} of {total}</span>
            <div style={{ display: 'flex', gap: 8 }}>
              <button disabled={skip === 0} onClick={() => setSkip(Math.max(0, skip - PAGE_SIZE))} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid #2d3e50', background: '#0d1b2a', color: skip === 0 ? '#2d3e50' : '#f5f0e8', cursor: skip === 0 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontSize: 13 }}><ChevronLeft size={14} /> Prev</button>
              <span style={{ padding: '6px 12px', fontSize: 13, color: '#a8b8c8' }}>{currentPage} / {totalPages}</span>
              <button disabled={skip + PAGE_SIZE >= total} onClick={() => setSkip(skip + PAGE_SIZE)} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid #2d3e50', background: '#0d1b2a', color: skip + PAGE_SIZE >= total ? '#2d3e50' : '#f5f0e8', cursor: skip + PAGE_SIZE >= total ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontSize: 13 }}>Next <ChevronRight size={14} /></button>
            </div>
          </div>
        )}
      </div>
